...
def _intensity_to_magnitude(label: str) -> float:
    label = str(label).strip().lower()
...
def _get_stance_label(stance_scores: dict) -> str:
    """
    Pick the dominant stance from NLI outputs.
    """
    if not stance_scores:
        return "neutral"

    pro = float(stance_scores.get("pro", 0.0))
    anti = float(stance_scores.get("anti", 0.0))
    neutral = float(stance_scores.get("neutral", 0.0))
...
def compute_target_axis_score(text: str, target: str) -> dict:
    """
    Returns target-specific score in [-10, +10].

    Positive => pro target
    Negative => anti target
    Zero     => neutral

    Important:
    - NLI decides only stance label
    - intensity decides only magnitude
    """
    text = clean_text(text)
    target = clean_text(target).lower()

    if not text or target not in TARGET_DISPLAY:
        return {
            "target": target,
            "label": "neutral",
            "score": 0.0,
            "stance_scores": {"pro": 0.0, "anti": 0.0, "neutral": 1.0},
            "intensity": "mild",
        }

    # Stage 1: target-specific stance
    stance_scores = _predict_stance_scores(text=text, target=target)
    stance_label = _get_stance_label(stance_scores)

    # Neutral means no signed contribution
    ...
    # Stage 2: intensity gives magnitude only
    intensity_label = _predict_intensity_label(text)
    magnitude = _intensity_to_magnitude(intensity_label)
    ...
    final_score = max(-10.0, min(10.0, float(final_score)))
